...
- 自动识别扩展名与实际格式不符的文件（如 .png 实为 JPEG），不改名直接按真实格式写入
- 自动重试存在次要元数据问题的文件
- 每次运行在输出目录生成修正报告 CSV，失败的文件标注 exiftool 的具体错误原因

### 无法处理的文件

文件本身损坏时（例如 HEIC 下载中断导致内部数据块截断），ExifTool 无法安全写入，报告中会标注具体错误，此类文件需从 iCloud 重新下载后再运行本工具。
